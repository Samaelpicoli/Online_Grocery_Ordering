...
class DirectoryManager:
    ...
    Esta classe permite a criação, busca, deleção, movimentação e monitoramento
    de arquivos em um diretório específico. Utiliza as bibliotecas padrão do Python
    para interagir com o sistema de arquivos e manipular arquivos de forma eficiente.
    ...
    def create_directory(self):
        ...
        Utiliza a função os.mkdir() para criar um novo diretório
        caso ele não exista.
        """
    ...
    def search_files(self, keyword: str = None) -> List[str]:
        """
        Procura por arquivos .xlsx no diretório base.
        ...
        Returns:
            list: Retorna uma lista com os nomes dos arquivos .xlsx ou 
            que contêm a palavra-chave encontrados no diretório.
        """
        if not keyword:
            pattern_for_search = '*.xlsx'
        else:
            pattern_for_search = f'*{keyword}*'

        file_paths = glob.glob(os.path.join(self.directory, pattern_for_search))
        filenames = [os.path.basename(file) for file in file_paths]
        return filenames
    

    def get_file(self, keyword: str = None) -> str:
        """
        Retorna o caminho completo do primeiro arquivo que contém a
        palavra-chave ou que seja .xlsx caso não tenha palavra-chave.
    ...
    def delete_file(self, path_or_file_name: str):
        ...
        Se for passado apenas o nome do arquivo, utiliza o diretório da instância.
